...
void P3RGB64x32MatrixPanel::drawPixel(int16_t x, int16_t y, uint16_t color) {
  if (x < 0 || x >= DWIDTH || y < 0 || y >= DHEIGHT) return;
  int16_t idx = x + y * DWIDTH;
  drawBuffer()[idx] = color;
}

uint16_t P3RGB64x32MatrixPanel::getPixel(int16_t x, int16_t y) {
  if (x < 0 || x >= DWIDTH || y < 0 || y >= DHEIGHT) 0;
  int16_t idx = x + y * DWIDTH;
  return drawBuffer()[idx];
}

void IRAM_ATTR P3RGB64x32MatrixPanel::draw() {
  static byte pheight = DHEIGHT-1;
  static byte pHeightShift = DHEIGHT;
  static byte cnt = 30;
  static byte y = 15;
  static uint32_t out = 0;
  y = (y + 1) % DISPLAYDEVIDER;
  ...
  for (int x = 0; x < DWIDTH; x++) {
    ...
    uint16_t c = matrixbuff[x + y * DWIDTH];
    r1 = (c & pheight) > cmp;
    g1 = ((c >>  5) & pheight) > cmp;
    b1 = ((c >> 10) & pheight) > cmp;
    c = matrixbuff[x + (y + 16) * DWIDTH];
    r2 = (c & pheight) > cmp;
    g2 = ((c >>  5) & pheight) > cmp;
    b2 = ((c >> 10) & pheight) > cmp;
  ...
  REG_WRITE(GPIO_OUT1_W1TS_REG, (1 << (pinOE - pHeightShift)) | (1 << (pinLAT - pHeightShift)));
  ...
  REG_WRITE(GPIO_OUT1_W1TC_REG, (1 << (pinOE - pHeightShift)) | (1 << (pinLAT - pHeightShift)));
}
