...
#define DWIDTH 128 // 128
#define DHEIGHT 32  // 32
#define DISPLAYDEVIDER 16  //16
#include <vector>
#include <array>
...
class P3RGB64x32MatrixPanel : public Adafruit_GFX {
  public:
    P3RGB64x32MatrixPanel(uint8_t _pinR1, uint8_t _pinG1, uint8_t _pinB1, uint8_t _pinR2, uint8_t _pinG2, uint8_t _pinB2, uint8_t _pinCLK, uint8_t _pinLAT, uint8_t _pinOE, uint8_t _pinA, uint8_t _pinB, uint8_t _pinC, uint8_t _pinD, bool _doubleBuffer = false)
      : Adafruit_GFX(DWIDTH, DHEIGHT), pinR1(_pinR1), pinG1(_pinG1), pinB1(_pinB1), pinR2(_pinR2), pinG2(_pinG2), pinB2(_pinB2), pinCLK(_pinCLK), pinLAT(_pinLAT), pinOE(_pinOE), pinA(_pinA), pinB(_pinB), pinC(_pinC), pinD(_pinD), doubleBuffer(_doubleBuffer) {
      initMatrixBuff();
    }
    P3RGB64x32MatrixPanel(bool _doubleBuffer = false)
      : Adafruit_GFX(DWIDTH, DHEIGHT), doubleBuffer(_doubleBuffer) {
      initMatrixBuff();
    }
    void begin(void);
    void stop(void);
    ...
    uint16_t color444(uint8_t r, uint8_t g, uint8_t b) { return ((g & 0xf) << 1) | ((uint16_t)(b & 0xf) << 6) | ((uint16_t)(r & 0xf) << 11); }
    uint16_t color555(uint8_t r, uint8_t g, uint8_t b) { return (g&0x1f) | ((uint16_t)(b & 0x1f) << 5) | ((uint16_t)(r & 0x1f) << 10); }
    ...
    uint16_t* matrixbuff;
    std::vector<std::array<uint16_t, DWIDTH*DHEIGHT>> _matrixbuff;
